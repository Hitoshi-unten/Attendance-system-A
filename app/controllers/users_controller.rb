class UsersController < ApplicationController
  ...
  # フォームを送信すると、フォームに含まれるフィールドはパラメータ（params）としてRailsに送信される。これらのパラメーターは、受けとっったコントローラ内のアクションで参照可能となっており、これを用いて特定のタスクを実行する。
  ...
  # paramsメソッドは今後多用することになるのでしっかり理解しておく。http://www.example.com/?username=dhh&email=dhh@email.comというURLで説明すると、params[:username]の値が「dhh」、params[:email]の値が「dhh@email.com」となる。
...
# 「リダイレクトするときは..._urlと指定する」とだけ覚えておく
 ...
 # そこで、コントローラで渡されるパラメータはホワイトリストでチェックし、不正なマスアサインメントを防がなければならない。この場合、createでパラメータを安全に扱うために、:name, :email, :department, :password, :password_confirmationパラメータの利用を「許可」し、かつ「必須」であることを指定したいのです。そのための構文によって、requireメソッドとpermitメソッドが導入される。
 # この記法を毎回繰り返すのは煩雑なので、たとえばcreateアクションとupdateアクションで共用できるようにこのメソッドをくくりだしておくのが普通。くくりだしたメソッドは、マスアサインメントを避けるだけでなく、外部から不正に呼び出されることのないようにprivate宣言の後に置いておく。
